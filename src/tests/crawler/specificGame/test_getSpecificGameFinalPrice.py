...
class TestGetSpecificGameFinalPrice(unittest.TestCase):
    @classmethod
    ...
    def test_getSpecificGameFinalPrice(self):
        """ Verifica se o método getSpecificGameFinalPrice() está funcionando 
        corretamente.

        """

        finalPrice = getSpecificGameFinalPrice(soup=self.soup, haveDiscount=self.haveDiscount, language=self.language)
    ...
    def test_getSpecificGameFinalPrice_exception(self, mockedObjectConstructor):
        """ Verifica se o método getSpecificGameFinalPrice() está capturando 
        corretamente a exceção.

        """

        mockedSoup = mockedObjectConstructor.return_value.raiseError.side_effect
        ret        = getSpecificGameFinalPrice(soup=mockedSoup, haveDiscount=self.haveDiscount, language=self.language)
